...
export const getPokemonImages = async (pokemonIds) => {
        ...
        const urls = pokemonIds.map(pokemonId => `https://pokeres.bastionbot.org/images/pokemon/${pokemonId}.png`)
...
export const getEvolutionChainData = async (chainId) => {
        const {data: pokemonEvolutionData} = await instance.get(`evolution-chain/${chainId}`);
